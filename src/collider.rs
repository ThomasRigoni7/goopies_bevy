...
pub fn check_collisions(
    mut query: Query<(&Transform, &mut Velocity), With<Collider>>,
) {
    ...
    for i in 0..transforms.len() {
        ...
        for j in 0..right.len() {
            ...
            assert_eq!(transform1.scale.x, transform1.scale.y, "Colliders must be circles. Found non-uniform scale on entity {}.", i);
            assert_eq!(transform2.scale.x, transform2.scale.y, "Colliders must be circles. Found non-uniform scale on entity {}.", j + i + 1);

            let circle1 = BoundingCircle {
                center: transform1.translation.truncate(),
                circle: Circle { radius: transform1.scale.x },
            };
            let circle2 = BoundingCircle {
                center: transform2.translation.truncate(),
                circle: Circle { radius: transform2.scale.x },
            };

            if let Some(collision) = collision(circle1, circle2) {
                println!("Collision detected between entity {} and entity {}: {:?}", i, j + i + 1, collision.direction);
...
pub fn collision(obj1: BoundingCircle, obj2: BoundingCircle) -> Option<Collision> {
    ...
    Some(Collision {
        direction: direction * ((obj1.circle.radius + obj2.circle.radius) / direction.length()) * 0.01
    })
}
